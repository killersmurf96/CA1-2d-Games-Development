...
bgImage.src = "images/backgrounds/background.png";
...
playerImage.src = "images/player/peter.png";
...
//item thaat will respawn randomelly 
var item = {
...
var menu = {
    ...
    draw: function () {
        ...
        if (menu.positionOfSelectionInMenu === 0) {
            ...
            menu.hl(1);
            g.fillText("Change Background", 10, 102);//writing chang background
            menu.hl(2);
...
function update(modifier) {
    ...
    if (32 in keysDown) { // Player holding space
        ...
        if (bulletInterval === null)
        {
            ...
            bullet.x = player.x;//sets bullet.x to player.x
            bullet.y = player.y;//sets bullet.y to player.y
            bulletFiring = false;
    ...
    if (player.y + player.size > canvas.height) {
        player.y = canvas.height - player.size;;//if the player.y + player.size greater then canvas width then player.y equals canvas.width - player.size
    }
    ;
...
function run() {//main game loop
    if (paused === false) {// if game is unpaused
        ...
        update((Date.now() - time) / 1000);//update everything
        render();//render
    }
